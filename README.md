...
| **Reflect** 反思 | p02, p08, p10, p14 | 3R 管线验证；检测母题缝隙并推荐改进方向；风格+母题组合改法；场景内因果诊断 |
| **Rewrite** 重写 | p07 | 以母题为约束引导生成，保持叙事一致 |
| **Reader** 读者 | p16 | 具名用户画像驱动的模拟读者群体仿真 |
...
p03 风格聚类 ─────────────────────────→ p09 审美复刻
           ...
           p05 提取 ──→ p06 跨作品识别 ──→ p07 约束生成 (Rewrite)

           p08 缝隙分析 ──→ p10 母题+风格合成
           (Reflect 环节)     (Reflect 环节)

p14 场景内诊断 ──→ p15 提纲生成 (场景辅助)
...
| p14 | 场景内行为链诊断 + 三策略修改建议 | 🟡 进行中 |
| p15 | 从 YAML+JSON 自动生成写作提纲 | 🟡 进行中 |
